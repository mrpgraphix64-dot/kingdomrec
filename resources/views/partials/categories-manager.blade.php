{{-- Manage Categories panel: opened from Job Post toolbar via window 'open-categories-manager' event --}}
<div x-data="categoryManager(@js($allCategories), @js($allSubCategories), @js($parentCategories))"
     @keydown.escape.window="modalOpen = false; subModalOpen = false; if(!modalOpen && !subModalOpen) panelOpen = false">

  <!-- ════════════════════════════════════ -->
  <!-- MANAGE CATEGORIES PANEL              -->
  <!-- ════════════════════════════════════ -->
  <div x-show="panelOpen" x-cloak class="fixed inset-0 z-[110] overflow-y-auto" style="display: none;">
      <div x-show="panelOpen" x-transition.opacity class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm" @click="panelOpen = false"></div>
      <div class="flex min-h-full items-start sm:items-center justify-center p-4 py-8">
          <div x-show="panelOpen" x-transition @click.stop class="relative w-full max-w-6xl bg-white rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">

              <!-- Panel Header -->
              <div class="px-6 py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50/50 rounded-t-2xl shrink-0">
                  <div>
                      <h2 class="text-lg font-bold text-kingdom-navy">Manage Categories</h2>
                  ...
                  <div class="flex items-center gap-2 shrink-0">
                      ...
                      <button @click="openCreateModal()"
                          class="bg-gradient-to-r from-kingdom-navy to-slate-800 hover:to-slate-900 text-white px-3 py-2 rounded-lg font-bold text-xs shadow-lg shadow-kingdom-navy/20 transition-all flex items-center gap-1.5 active:scale-95">
                          <i data-lucide="plus" class="w-3.5 h-3.5"></i>
                          <span class="hidden sm:inline">Add</span> Category
                      </button>
                      <button @click="panelOpen = false" class="bg-white p-2 rounded-full text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors shadow-sm border border-slate-100">
              ...
              <div class="p-6 overflow-y-auto flex-1">
                  ...
                  <!-- Stats Overview -->
                  <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 lg:gap-4 shrink-0 mb-4">
                  ...
                  <hr class="border-slate-200 my-2 shrink-0">

                  <!-- Categories Grid List -->
                  <div class="anim-fade-in-up grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pb-2 items-start">
  ...
  <!-- ════════════════════════════════════ -->
  <div x-show="modalOpen" class="fixed inset-0 z-[120] overflow-y-auto" style="display: none;">
  ...
  <!-- ════════════════════════════════════ -->
  <div x-show="subModalOpen" class="fixed inset-0 z-[120] overflow-y-auto" style="display: none;">
  ...
  <script>
    window.categoryManager = function categoryManager(categories, subCategories, parentCategories) {
        return {
            // Panel visibility (opened from Job Post toolbar)
            panelOpen: false,
            ...
            init() {
                window.addEventListener('open-categories-manager', () => { this.panelOpen = true; });
                this.$watch('panelOpen', val => { document.body.style.overflow = (val || this.modalOpen || this.subModalOpen) ? 'hidden' : ''; });
                this.$watch('modalOpen', val => { document.body.style.overflow = (val || this.panelOpen || this.subModalOpen) ? 'hidden' : ''; });
                this.$watch('subModalOpen', val => { document.body.style.overflow = (val || this.panelOpen || this.modalOpen) ? 'hidden' : ''; });
